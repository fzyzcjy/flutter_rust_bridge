...
class BenchConfig {
  ...
  const BenchConfig({required this.partialName});
}
...
Future<void> benchDartNative(BenchConfig config) async {
  ...
  await exec(
      ...
      relativePwd: package);
  await exec(
      'build/simple_benchmark/simple_benchmark.exe build/simple_benchmark/benchmark_result.json ${config.partialName}',
      relativePwd: package);
}
