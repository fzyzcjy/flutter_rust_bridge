...
impl WorkerPool {
    ...
    fn try_get_worker(&self) -> Option<Worker> {
        self.state.workers.borrow_mut().pop()
    }
    ...
    /// ## Returns
    ///
    /// - `Ok(ExecuteResult::Executed(worker))` - Work was executed on the returned worker
    /// - `Ok(ExecuteResult::Queued)` - Work was queued for later execution (when max workers reached)
    /// - `Err(e)` - An error occurred while creating worker or sending message
    ...
    fn execute_raw(&self, closure: TransferClosure<JsValue>) -> Result<ExecuteResult, JsValue> {
        if let Some(worker) = self.try_get_worker() {
            closure.apply(&worker)?;
            return Ok(ExecuteResult::Executed(worker));
        }
        ...
        if self.state.max_workers.is_some() {
            self.state.work_queue.borrow_mut().push_back(PendingWork { closure });
            return Ok(ExecuteResult::Queued);
        }
        
        Err("Failed to spawn worker".into())
    }
