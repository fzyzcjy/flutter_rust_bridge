...
impl<T: DartSafe> RustOpaque<RwLock<T>> {
    pub fn rust_auto_opaque_cst_decode_owned(self) -> Result<T> {
        self.into_inner()
    ...
    pub fn rust_auto_opaque_cst_decode_ref(&self) -> Result<RwLockReadGuard<'_, T>> {
        self.read().map_err(|_| anyhow!("lock is poisioned"))
    }

    pub fn rust_auto_opaque_cst_decode_ref_mut(&self) -> Result<RwLockWriteGuard<'_, T>> {
