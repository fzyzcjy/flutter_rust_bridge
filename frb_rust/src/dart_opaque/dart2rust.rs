...
unsafe fn dart_opaque_dart2rust_encode_inner(
    ...
    println!(
        "hi dart_opaque_dart2rust_encode_inner BEFORE handle={handle:?} thread_id={:?} Dart_CurrentIsolate={:?} backtrace={:?}",
    ...
    println!(
        "hi dart_opaque_dart2rust_encode_inner AFTER handle={handle:?} ans={ans:?} thread_id={:?} Dart_CurrentIsolate={:?} backtrace={:?}",
        std::thread::current().id(),
        dart_sys::Dart_CurrentIsolate_DL.unwrap()(),
        std::backtrace::Backtrace::force_capture(),
    );
