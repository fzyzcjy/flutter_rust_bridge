...
- String handles contain the physical channel name and logical port name. Each Rust thread reuses its own BroadcastChannel sender within a synchronous batch, including custom thread pools and nested workers. Sending does not depend on the parent worker's event loop.
...
- The physical receiver remains alive for the Dart context's lifetime. A microtask closes Rust senders after each synchronous batch so idle workers do not remain broadcast recipients. Creating or closing a stream does not register another physical receiver. Only initialization readiness probes are repeated; stream data is never retried.
