...
- String handles contain the physical channel name and logical port name. Each Rust thread reuses its own BroadcastChannel sender for the worker context's lifetime, including custom thread pools and nested workers. Sending does not depend on the parent worker's event loop.
...
- Sink clones share one channel handle and sequence counter through their closer. Values and the final close use that handle and carry sequence numbers, and Dart delivers them in that order, including when BroadcastChannel delivers frames out of order.
...
- The physical receiver remains alive for the Dart context's lifetime. Cached senders remain broadcast recipients while their worker contexts live. Creating or closing a stream does not register another physical receiver. Only initialization readiness probes are repeated; stream data is never retried.
