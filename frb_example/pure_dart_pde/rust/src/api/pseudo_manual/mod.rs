// AUTO-GENERATED BY frb_internal, PLEASE DO NOT EDIT
pub mod array_twin_rust_async;
pub mod array_twin_sync;
pub mod attribute_twin_rust_async;
pub mod attribute_twin_sync;
pub mod basic;
pub mod basic_list;
pub mod basic_list_twin_rust_async;
pub mod basic_list_twin_sync;
pub mod basic_map;
pub mod basic_map_twin_rust_async;
pub mod basic_map_twin_sync;
pub mod basic_optional;
pub mod basic_optional_twin_rust_async;
pub mod basic_optional_twin_sync;
pub mod basic_twin_rust_async;
pub mod basic_twin_sync;
pub mod benchmark_api;
pub mod benchmark_api_twin_rust_async;
pub mod benchmark_api_twin_sync;
pub mod chrono_type_twin_rust_async;
pub mod chrono_type_twin_sync;
pub mod collection_equality_twin_rust_async;
pub mod collection_equality_twin_sync;
pub mod comment_twin_rust_async;
pub mod comment_twin_sync;
pub mod dart_fn_twin_rust_async;
pub mod dart_opaque_twin_rust_async;
pub mod dart_opaque_twin_sync;
pub mod dropping_twin_rust_async;
pub mod dropping_twin_sync;
pub mod enumeration_twin_rust_async;
pub mod enumeration_twin_sync;
pub mod event_listener_twin_rust_async;
pub mod exception_twin_rust_async;
pub mod exception_twin_sync;
pub mod external_type_in_crate_twin_rust_async;
pub mod external_type_in_crate_twin_sync;
pub mod impl_trait_twin_sync;
pub mod lifetimeable_twin_sync;
pub mod map_and_set_twin_rust_async;
pub mod map_and_set_twin_sync;
pub mod method_twin_rust_async;
pub mod method_twin_sync;
pub mod mirror_twin_rust_async;
pub mod mirror_twin_sync;
pub mod misc_example_twin_rust_async;
pub mod misc_example_twin_sync;
pub mod misc_type_twin_rust_async;
pub mod misc_type_twin_sync;
pub mod namespaced_type_twin_rust_async;
pub mod namespaced_type_twin_sync;
pub mod newtype_pattern_twin_rust_async;
pub mod newtype_pattern_twin_sync;
pub mod optional_primitive_misc_twin_rust_async;
pub mod optional_primitive_misc_twin_sync;
pub mod optional_twin_rust_async;
pub mod optional_twin_sync;
pub mod ownership_twin_rust_async;
pub mod ownership_twin_sync;
pub mod primitive_list_misc_twin_rust_async;
pub mod primitive_list_misc_twin_sync;
pub mod primitive_misc_twin_rust_async;
pub mod primitive_misc_twin_sync;
pub mod raw_string_twin_rust_async;
pub mod raw_string_twin_sync;
pub mod rust_auto_opaque_twin_rust_async;
pub mod rust_auto_opaque_twin_sync;
pub mod rust_opaque_twin_rust_async;
pub mod rust_opaque_twin_sync;
pub mod serde_json_type_twin_rust_async;
pub mod serde_json_type_twin_sync;
pub mod simple_twin_rust_async;
pub mod simple_twin_sync;
pub mod stream_twin_rust_async;
pub mod structure_twin_rust_async;
pub mod structure_twin_sync;
pub mod tuple_twin_rust_async;
pub mod tuple_twin_sync;
pub mod type_alias_twin_rust_async;
pub mod type_alias_twin_sync;
pub mod uuid_type_twin_rust_async;
pub mod uuid_type_twin_sync;
