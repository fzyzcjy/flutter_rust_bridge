...
lazy_static! {
    static ref EVENTS: Mutex<Option<StreamSinkImpl<EventTwinSync>>> = Default::default();
}
...
#[flutter_rust_bridge::frb(sync)]
pub fn register_event_listener_twin_sync(listener: impl StreamSink<EventTwinSync>) -> Result<()> {
    match EVENTS.lock() {
        Ok(mut guard) => {
            *guard = Some(listener);
            Ok(())
