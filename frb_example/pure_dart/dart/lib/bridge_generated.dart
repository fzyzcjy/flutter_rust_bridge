...
class FlutterRustBridgeExampleSingleBlockTestImpl implements FlutterRustBridgeExampleSingleBlockTest {
  ...
  Future<TestBag> test42({dynamic hint}) => _platform.executeNormal(FlutterRustBridgeTask(
        callFfi: (port_) => _platform.inner.wire_test42(port_),
        parseSuccessData: _wire2api_box_test_bag,
        constMeta: kTest42ConstMeta,
        argValues: [],
        hint: hint,
      ));

  FlutterRustBridgeTaskConstMeta get kTest42ConstMeta => const FlutterRustBridgeTaskConstMeta(
        debugName: "test42",
        argNames: [],
      );
...
TestBag _wire2api_box_test_bag(dynamic raw) {
  return _wire2api_test_bag(raw);
}
...
TestBag _wire2api_test_bag(dynamic raw) {
  final arr = raw as List<dynamic>;
  if (arr.length != 1) throw Exception('unexpected arr length: expect 1 but see ${arr.length}');
  return TestBag(
    test: _wire2api_String(arr[0]),
  );
}
