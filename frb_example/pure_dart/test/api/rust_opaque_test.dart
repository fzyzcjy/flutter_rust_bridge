...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('Web MoiArc release completes after pool contention', () async {
    await Future<void>.sync(reproduceMoiArcReleaseContentionTwinNormal);
    final deadline = DateTime.now().add(const Duration(seconds: 5));
    while (await moiArcContentionValueDropCountTwinNormal() == 0 &&
        DateTime.now().isBefore(deadline)) {
      await Future<void>.delayed(const Duration(milliseconds: 10));
    }
    expect(await moiArcContentionValueDropCountTwinNormal(), 1);
  }, skip: !kIsWeb);
  ...
  test('call after dispose', () async {
    ...
    data.dispose();
    await expectLater(
      () => runOpaqueTwinNormal(opaque: data),
  ...
  test('create enums of opaque type', () async {
    ...
    await expectLater(
      () => runEnumOpaqueTwinNormal(opaque: data[4]),
      throwsA(isA<DroppableDisposedException>()),
    );
  });
  ...
  test('opaque field', () async {
    ...
    data.first.dispose();
    await expectLater(
      () => runOpaqueTwinNormal(opaque: data.first),
      throwsA(isA<DroppableDisposedException>()),
    );
    await expectLater(
      () => runNestedOpaqueTwinNormal(opaque: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    expect(
      await runOpaqueTwinNormal(opaque: data.second),
  ...
  test('array', () async {
    ...
    await expectLater(
      () => opaqueArrayRunTwinNormal(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
  ...
  test('vec', () async {
    ...
    await expectLater(
      () => opaqueVecRunTwinNormal(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
