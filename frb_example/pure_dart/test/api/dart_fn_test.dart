...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('rustCallDartOptionalTwinNormal', () async {
    final callbackCalls = <String>[];

    await rustCallDartOptionalTwinNormal(callback: callbackCalls.add);
    await rustCallDartOptionalTwinNormal(callback: null);
  ...
  test('rustCallDartTwoArgsTwinNormal', () async {
    ...
    expect(callbackCalls, [
      ('a', DemoStructForRustCallDartTwinNormal(name: 'b')),
    ]);
  });

  test('rustCallDartReturnTwinNormal', () async {
    var callCount = 0;
    await rustCallDartReturnTwinNormal(
      callback: () {
        callCount++;
        return 'a';
      },
  ...
  test('rustCallDartLoopbackTwinNormal', () async {
    var callCount = 0;
    await rustCallDartLoopbackTwinNormal(
      callback: (arg) {
        callCount++;
        return arg;
      },
  ...
  test('rustCallDartWithDartOpaqueResultTwinNormal', () async {
    final opaque = (String whatever) => 42;
    var callCount = 0;
    final dynamic output = await rustCallDartWithDartOpaqueResultTwinNormal(
      callback: () {
        callCount++;
        return opaque;
      },
