...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('Web MoiArc release completes after pool contention', () async {
    await Future<void>.sync(reproduceMoiArcReleaseContentionTwinSyncMoi);
    final deadline = DateTime.now().add(const Duration(seconds: 5));
    while (await moiArcContentionValueDropCountTwinSyncMoi() == 0 &&
        DateTime.now().isBefore(deadline)) {
      await Future<void>.delayed(const Duration(milliseconds: 10));
    }
    expect(await moiArcContentionValueDropCountTwinSyncMoi(), 1);
  }, skip: !kIsWeb);
  ...
  test('call after dispose', () async {
    ...
    data.dispose();
    await expectLater(
      () => runOpaqueTwinSyncMoi(opaque: data),
  ...
  test('create enums of opaque type', () async {
    ...
    await expectLater(
      () => runEnumOpaqueTwinSyncMoi(opaque: data[4]),
      throwsA(isA<DroppableDisposedException>()),
    );
  });
  ...
  test('opaque field', () async {
    ...
    data.first.dispose();
    await expectLater(
      () => runOpaqueTwinSyncMoi(opaque: data.first),
      throwsA(isA<DroppableDisposedException>()),
    );
    await expectLater(
      () => runNestedOpaqueTwinSyncMoi(opaque: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    expect(
      await runOpaqueTwinSyncMoi(opaque: data.second),
  ...
  test('array', () async {
    ...
    await expectLater(
      () => opaqueArrayRunTwinSyncMoi(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
  ...
  test('vec', () async {
    ...
    await expectLater(
      () => opaqueVecRunTwinSyncMoi(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
