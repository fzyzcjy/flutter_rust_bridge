...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  addTestsIdentityFunctionCall(funcEnumWithItemTupleTwinSse, [
    ...
    EnumWithItemTupleTwinSse.b(200),
  ]);
  ...
  addTestsIdentityFunctionCall(funcEnumWithFreezedOverrideTwinSse, [
    const EnumWithFreezedOverrideTwinSse.empty(),
    const EnumWithFreezedOverrideTwinSse.named(count: 42),
    const EnumWithFreezedOverrideTwinSse.named(),
  ]);

  group('example-based tests', () {
    test('dart call handleReturnEnum', () async {
      expect(
        await handleReturnEnumTwinSse(input: "Tuesday"),
    ...
    test('dart call multiplyByTen()', () async {
      ...
      expect(
        (SpeedTwinSse_Unknown).toString(),
        'SpeedTwinSse_Unknown',
        skip: skipMinified,
      );
      expect(
        (SpeedTwinSse_GPS).toString(),
        'SpeedTwinSse_GPS',
        skip: skipMinified,
      );
      expect(
        (DistanceTwinSse_Unknown).toString(),
        'DistanceTwinSse_Unknown',
        skip: skipMinified,
      );
      expect(
        (DistanceTwinSse_Map).toString(),
        'DistanceTwinSse_Map',
        skip: skipMinified,
      );
    });
