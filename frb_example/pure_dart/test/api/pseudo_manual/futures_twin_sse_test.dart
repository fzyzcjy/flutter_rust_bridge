...
import 'package:frb_example_pure_dart/src/rust/api/pseudo_manual/futures_twin_sse.dart';
...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  group('functions returning futures', () {
    test('dart call `impl Future` adder', () async {
      expect(await implFutureAdderTwinSse(a: 100, b: 200), 300);
    });

    test('dart call `DartFnFuture` adder', () async {
      expect(await dartfnFutureAdderTwinSse(a: 100, b: 200, c: 300), 600);
    });
    ...
    test('dart call `impl Future` adder returning `Result::Ok`', () async {
      expect(await implFutureAdderResultTwinSse(a: 100, b: 200, true), 300);
    });

    test('dart call `impl Future` adder returning `Result::Err`', () async {
      expect(await expectLater(
          () async => implFutureAdderResultTwinSse(a: 100, b: 200, false),
          throwsA(isA<CustomErrTwinSse>())));
    });

    test('dart call `DartFnFuture` adder returning `Result::Ok`', () async {
      expect(await dartfnFutureAdderResultTwinSse(a: 100, b: 200, c: 300, true),
          600);
    });
    ...
    test('dart call `DartFnFuture` adder returning `Result::Err`', () async {
      expect(await expectLater(
          () async =>
              dartfnFutureAdderResultTwinSse(a: 100, b: 200, c: 300, false),
          throwsA(isA<CustomErrTwinSse>())));
    });

    test('dart call `BoxFuture` adder returning `Result::Ok`', () async {
      expect(await boxFutureAdderTwinSse(a: 100, b: 200, c: 300, d: 400, true),
          1000);
    });
    ...
    test('dart call `BoxFuture` adder returning `Result::Err`', () async {
      expect(await expectLater(
          () async =>
              boxFutureAdderTwinSse(a: 100, b: 200, c: 300, d: 400, false),
          throwsA(isA<CustomErrTwinSse>())));
    });
  });
