...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  group('systematic test', () {
    ...
    test('call funcTypeFalliblePanicTwinSse', skip: kIsWeb, () async {
      await expectRustPanic(
        () async => funcTypeFalliblePanicTwinSse(),
        'TwinSse',
        messageOnNative: 'deliberate panic',
      );
    });
    test('call funcTypeInfalliblePanicTwinSse', skip: kIsWeb, () async {
      await expectRustPanic(
        () async => funcTypeInfalliblePanicTwinSse(),
        'TwinSse',
        messageOnNative: 'deliberate panic',
      );
    });
    ...
    test('call customEnumErrorPanicTwinSse', skip: kIsWeb, () async {
      await expectRustPanic(
        () async => customEnumErrorPanicTwinSse(),
        'TwinSse',
        messageOnNative: 'deliberate panic',
      );
    });
    ...
    test('call funcReturnErrorTwinSse', () async {
      var matcher = isA<CustomEnumErrorTwinSse>().having(
        (x) => x.message,
        'message',
        startsWith('deliberate error'),
      );
      ...
      await expectLater(
        () async => customEnumErrorReturnErrorTwinSse(),
        throwsA(matcher),
      );
    });
  ...
  group('example-based tests', () {
    group('custom errors', () {
      ...
      test('Throw CustomError', timeout: Timeout.factor(5), () {
        expect(
          () async => returnErrCustomErrorTwinSse(),
      ...
      test('Throw CustomStructError', () async {
        await expectLater(
          () async => returnCustomStructErrorTwinSse(),
      ...
      test('Throw CustomNestedError1', () async {
        await expectLater(
          ...
          throwsA(
            CustomNestedError1TwinSse.errorNested(
      ...
      test('Throw CustomNestedError2', () async {
        await expectLater(
          () async => returnCustomNestedError2TwinSse(),
      ...
      test('Throw CustomError variant 0', () async {
        await expectLater(
          () async => returnErrorVariantTwinSse(variant: 0),
          throwsA(isA<CustomErrorTwinSse>()),
        );
      });
      ...
      test('Throw CustomError variant 1', () async {
        await expectLater(
          () async => returnErrorVariantTwinSse(variant: 1),
          throwsA(isA<CustomErrorTwinSse>()),
        );
      });
      ...
      test('Do not throw CustomError static method', () async {
        expect(
          await SomeStructTwinSse.staticReturnOkCustomErrorTwinSse(),
          3,
        );
      });
      ...
      test('Function with custom result panics', () async {
        await expectRustPanicRaw(
          () async => panicWithCustomResultTwinSse(),
          'TwinSse',
          throwsA(isA<FrbException>()),
        );
      });
