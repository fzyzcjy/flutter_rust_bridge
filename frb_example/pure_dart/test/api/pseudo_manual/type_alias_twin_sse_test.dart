...
import 'package:frb_example_pure_dart/src/rust/api/pseudo_manual/type_alias_twin_sse.dart';
...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('dart call handle_type_id', () async {
    final id = await handleTypeAliasIdTwinSse(input: BigInt.from(42));
    expect(id.toInt(), 42);
  });

  test('dart call handle_type_nest_alias_id', () async {
    final id = await handleTypeNestAliasIdTwinSse(input: BigInt.from(42));
    expect(id.toInt(), 42);
  });

  test('dart call handle_type_model', () async {
    final testModel = await handleTypeAliasModelTwinSse(
  ...
  test('infallible API compiles when user shadows std Result (#1710)',
      () async {
    final value = await infallibleWithResultShadowTwinSse();
    expect(value, 42);
  });
