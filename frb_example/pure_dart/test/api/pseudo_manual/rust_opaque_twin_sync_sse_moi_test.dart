...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('Web MoiArc release completes after pool contention', () async {
    await Future<void>.sync(reproduceMoiArcReleaseContentionTwinSyncSseMoi);
    final deadline = DateTime.now().add(const Duration(seconds: 5));
    while (await moiArcContentionValueDropCountTwinSyncSseMoi() == 0 &&
        DateTime.now().isBefore(deadline)) {
      await Future<void>.delayed(const Duration(milliseconds: 10));
    }
    expect(await moiArcContentionValueDropCountTwinSyncSseMoi(), 1);
  }, skip: !kIsWeb);
  ...
  test('call after dispose', () async {
    ...
    data.dispose();
    await expectLater(
      () => runOpaqueTwinSyncSseMoi(opaque: data),
  ...
  test('create array of opaque type', () async {
    ...
    for (var v in data) {
      ...
      v.dispose();
      await expectLater(
        () => runOpaqueTwinSyncSseMoi(opaque: v),
        throwsA(isA<DroppableDisposedException>()),
      );
    }
  });
  ...
  test('create enums of opaque type', () async {
    ...
    await expectLater(
      () => runEnumOpaqueTwinSyncSseMoi(opaque: data[4]),
      throwsA(isA<DroppableDisposedException>()),
    );
  });
  ...
  test('opaque field', () async {
    ...
    data.first.dispose();
    await expectLater(
      () => runOpaqueTwinSyncSseMoi(opaque: data.first),
      throwsA(isA<DroppableDisposedException>()),
    );
    await expectLater(
      () => runNestedOpaqueTwinSyncSseMoi(opaque: data),
  ...
  test('array', () async {
    ...
    await expectLater(
      () => opaqueArrayRunTwinSyncSseMoi(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
  ...
  test('vec', () async {
    ...
    await expectLater(
      () => opaqueVecRunTwinSyncSseMoi(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
