...
import 'package:frb_example_pure_dart/src/rust/api/pseudo_manual/optional_twin_rust_async_sse.dart';
...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('dart call handleOptionalReturn', () async {
    expect((await handleOptionalReturnTwinRustAsyncSse(left: 1, right: 1))!, 1);
    expect(await handleOptionalReturnTwinRustAsyncSse(left: 2, right: 0), null);
  });

  test('dart call handleOptionalStruct', () async {
    {
      expect(await handleOptionalStructTwinRustAsyncSse(), null);
    }

    {
      final message = 'Hello there.';
      final ret = await handleOptionalStructTwinRustAsyncSse(document: message);
      if (ret == null) {
  ...
  test('dart call handleOptionalIncrement', () async {
    expect(await handleOptionalIncrementTwinRustAsyncSse(), null);
    {
      var ret = await handleOptionalIncrementTwinRustAsyncSse(
          opt: ExoticOptionalsTwinRustAsyncSse(attributesNullable: []));
      if (ret == null) fail('increment returned null for non-null params');
      final loopFor = 20;
      for (var i = 1; i < loopFor; i++) {
        ret = await handleOptionalIncrementTwinRustAsyncSse(opt: ret);
      }
  ...
  test('dart call handleIncrementBoxedOptional', () async {
    {
      expect(await handleIncrementBoxedOptionalTwinRustAsyncSse(), 42);
    }

    {
      ...
      for (var i = 0; i < loopFor; i++) {
        ret = await handleIncrementBoxedOptionalTwinRustAsyncSse(opt: ret);
      }
      expect(ret, loopFor);
    }
  });
  ...
  test('dart call handleOptionBoxArguments', () async {
    print(await handleOptionBoxArgumentsTwinRustAsyncSse());

    {
      final optional10 = await handleOptionBoxArgumentsTwinRustAsyncSse(
        boolbox: true,
        structbox: await handleOptionalIncrementTwinRustAsyncSse(
            opt: ExoticOptionalsTwinRustAsyncSse(attributesNullable: [])),
      );
      print(optional10);
    }
