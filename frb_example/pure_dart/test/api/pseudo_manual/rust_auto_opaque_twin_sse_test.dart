...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  group('simple functions', () {
    ...
    group('arg owned', () {
      ...
      test('failed encoding preserves an earlier owned argument', () async {
        final first = await rustAutoOpaqueReturnOwnTwinSse(initial: 10);
        final disposed = await rustAutoOpaqueReturnOwnTwinSse(initial: 20);
        disposed.dispose();

        await expectLater(
          () => rustAutoOpaqueTwoArgsTwinSse(a: first, b: disposed),
          throwsA(isA<DroppableDisposedException>()),
        );
        expect(first.isDisposed, false);
        await futurizeVoidTwinSse(
          rustAutoOpaqueArgOwnTwinSse(arg: first, expect: 10),
        );
      });

      test('failed duplicate move preserves the owned argument', () async {
        final obj = await rustAutoOpaqueReturnOwnTwinSse(initial: 10);

        await expectLater(
          () => rustAutoOpaqueTwoArgsTwinSse(a: obj, b: obj),
          throwsA(_duplicateMoveErrorMatcher()),
        );
        expect(obj.isDisposed, false);
        await futurizeVoidTwinSse(
          rustAutoOpaqueArgOwnTwinSse(arg: obj, expect: 10),
        );
      });
    });
    ...
    group('arg ref mut', () {
      ...
      test('after call, the object can still be used again', () async {
        ...
        await futurizeVoidTwinSse(
          rustAutoOpaqueArgMutBorrowTwinSse(
            arg: obj,
            expect: 101,
            adder: 10,
          ),
  ...
  group('Explicit rust-auto-opaque types', () {
    ...
    test('it can be inside a struct used as argument', () async {
      final obj = await rustAutoOpaqueExplicitReturnTwinSse(initial: 100);
      await futurizeVoidTwinSse(
        rustAutoOpaqueExplicitStructTwinSse(
    ...
    test('it can be inside a struct used as return type', () async {
      ...
      await futurizeVoidTwinSse(
        rustAutoOpaqueExplicitStructTwinSse(arg: obj),
      );
    });
  ...
  group('borrow + borrow', () {
    test('when same object', () async {
      final obj = await rustAutoOpaqueReturnOwnTwinSse(initial: 100);
      expect(
        await rustAutoOpaqueBorrowAndBorrowTwinSse(a: obj, b: obj),
        200,
      );
    });
