...
import 'package:frb_example_pure_dart/src/rust/api/pseudo_manual/misc_type_twin_rust_async_sse.dart';
...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  addTestsIdentityFunctionCall(funcStringTwinRustAsyncSse, ['', 'hello', '😂']);

  test('call funcReturnUnitTwinRustAsyncSse', () async {
    // `as dynamic` to make the generated pseudo_manual test happy
    await (funcReturnUnitTwinRustAsyncSse() as dynamic);
  });

  test('dart call handleListOfStruct', () async {
    final listOfStructResp = await handleListOfStructTwinRustAsyncSse(
  ...
  test('dart call handleStringList', () async {
    final names =
        await handleStringListTwinRustAsyncSse(names: ['Steve', 'Bob', 'Alex']);
    expect(names, ['Steve', 'Bob', 'Alex']);
  });

  test('dart call handleVecOfOpts', () async {
    const loops = 20;
    var opt = OptVecsTwinRustAsyncSse(
        i32: [],
        enums: [WeekdaysTwinRustAsyncSse.monday],
        strings: ['foo'],
        buffers: []);
    for (var i = 0; i < loops; i++) {
      opt = await handleVecOfOptsTwinRustAsyncSse(opt: opt);
    }
    final nulls = List.filled(loops, null);
    expect(opt.i32, nulls);
    expect(opt.enums,
        [WeekdaysTwinRustAsyncSse.monday, for (final val in nulls) val]);
    expect(opt.strings, ['foo', for (final val in nulls) val]);
    expect(opt.buffers, nulls);
  });

  test('test empty struct', () async {
    final empty = EmptyTwinRustAsyncSse();
    final output = await emptyStructTwinRustAsyncSse(empty: empty);
    expect(output, isA<EmptyTwinRustAsyncSse>());
  });
}
