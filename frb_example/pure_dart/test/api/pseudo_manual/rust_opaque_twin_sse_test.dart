...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('Web MoiArc release completes after pool contention', () async {
    await Future<void>.sync(reproduceMoiArcReleaseContentionTwinSse);
    final deadline = DateTime.now().add(const Duration(seconds: 5));
    while (await moiArcContentionValueDropCountTwinSse() == 0 &&
        DateTime.now().isBefore(deadline)) {
      await Future<void>.delayed(const Duration(milliseconds: 10));
    }
    expect(await moiArcContentionValueDropCountTwinSse(), 1);
  }, skip: !kIsWeb);
  ...
  test('create enums of opaque type', () async {
    ...
    await expectLater(
      () => runEnumOpaqueTwinSse(opaque: data[4]),
      throwsA(isA<DroppableDisposedException>()),
    );
  });
  ...
  test('opaque field', () async {
    ...
    data.first.dispose();
    await expectLater(
      () => runOpaqueTwinSse(opaque: data.first),
      throwsA(isA<DroppableDisposedException>()),
    );
    await expectLater(
      () => runNestedOpaqueTwinSse(opaque: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    expect(
      await runOpaqueTwinSse(opaque: data.second),
  ...
  test('array', () async {
    ...
    await expectLater(
      () => opaqueArrayRunTwinSse(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
