...
import 'package:frb_example_pure_dart/src/rust/api/pseudo_manual/serde_json_type_twin_sync_sse.dart';
...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('serde_json::Value null', () async {
    final output = await handleSerdeJsonValueTwinSyncSse(val: null);
    expect(output, isNull);
  });

  test('serde_json::Value number', () async {
    final output = await handleSerdeJsonValueTwinSyncSse(val: 42);
    expect(output, 42);
  });

  test('serde_json::Value string', () async {
    final output = await handleSerdeJsonValueTwinSyncSse(val: 'hello');
    expect(output, 'hello');
  });

  test('serde_json::Value bool', () async {
    final output = await handleSerdeJsonValueTwinSyncSse(val: true);
    expect(output, true);
  });

  test('serde_json::Value list', () async {
    final input = [1, 'two', null, true];
    final output = await handleSerdeJsonValueTwinSyncSse(val: input);
    expect(output, input);
  });
  ...
  test('Option<serde_json::Value> some', () async {
    final output =
        await handleOptionSerdeJsonValueTwinSyncSse(val: {'key': 'value'});
    expect(output, {'key': 'value'});
  });

  test('Option<serde_json::Value> none', () async {
    final output = await handleOptionSerdeJsonValueTwinSyncSse(val: null);
    expect(output, isNull);
  });
  ...
  test('Vec<serde_json::Value>', () async {
    ...
    final output = await handleVecSerdeJsonValueTwinSyncSse(val: input);
    expect(output, input);
  });
  ...
  test('HashMap<String, serde_json::Value>', () async {
    ...
    final output = await handleMapSerdeJsonValueTwinSyncSse(val: input);
    expect(output, input);
  });
  ...
  test('serde_json::Value nested struct', () async {
    ...
    final wrapper = FeatureSerdeJsonTwinSyncSse(data: data);
    final output =
        await handleNestedSerdeJsonValueTwinSyncSse(wrapper: wrapper);
    expect(output.data, data);
  });

  test('Big integer and decimal Rust library types', () async {
    final input = FeatureBigNumberTwinSyncSse(
    ...
    final output = await handleBigNumberTypesTwinSyncSse(input: input);
