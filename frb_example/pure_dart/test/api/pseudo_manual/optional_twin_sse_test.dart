...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('dart call handleOptionalReturn', () async {
    expect((await handleOptionalReturnTwinSse(left: 1, right: 1))!, 1);
    expect(await handleOptionalReturnTwinSse(left: 2, right: 0), null);
  });

  test('dart call handleOptionalStruct', () async {
    {
      expect(await handleOptionalStructTwinSse(), null);
    }

    {
      final message = 'Hello there.';
      final ret = await handleOptionalStructTwinSse(document: message);
      if (ret == null) {
  ...
  test('dart call handleOptionalIncrement', () async {
    expect(await handleOptionalIncrementTwinSse(), null);
    {
      var ret = await handleOptionalIncrementTwinSse(
          opt: ExoticOptionalsTwinSse(attributesNullable: []));
      if (ret == null) fail('increment returned null for non-null params');
      final loopFor = 20;
      for (var i = 1; i < loopFor; i++) {
        ret = await handleOptionalIncrementTwinSse(opt: ret);
      }
  ...
  test('dart call handleIncrementBoxedOptional', () async {
    {
      expect(await handleIncrementBoxedOptionalTwinSse(), 42);
    }

    {
      ...
      for (var i = 0; i < loopFor; i++) {
        ret = await handleIncrementBoxedOptionalTwinSse(opt: ret);
      }
      expect(ret, loopFor);
    }
  });
  ...
  test('dart call handleOptionBoxArguments', () async {
    print(await handleOptionBoxArgumentsTwinSse());

    {
      final optional10 = await handleOptionBoxArgumentsTwinSse(
        boolbox: true,
        structbox: await handleOptionalIncrementTwinSse(
            opt: ExoticOptionalsTwinSse(attributesNullable: [])),
      );
      print(optional10);
    }
