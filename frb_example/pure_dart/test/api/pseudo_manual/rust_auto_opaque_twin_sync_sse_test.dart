...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  group('simple functions', () {
    ...
    group('arg owned', () {
      ...
      test('failed encoding preserves an earlier owned argument', () async {
        final first = await rustAutoOpaqueReturnOwnTwinSyncSse(initial: 10);
        final disposed = await rustAutoOpaqueReturnOwnTwinSyncSse(initial: 20);
        disposed.dispose();

        await expectLater(
          () => rustAutoOpaqueTwoArgsTwinSyncSse(a: first, b: disposed),
          throwsA(isA<DroppableDisposedException>()),
        );
        expect(first.isDisposed, false);
        await futurizeVoidTwinSyncSse(
          rustAutoOpaqueArgOwnTwinSyncSse(arg: first, expect: 10),
        );
      });

      test('failed duplicate move preserves the owned argument', () async {
        final obj = await rustAutoOpaqueReturnOwnTwinSyncSse(initial: 10);

        await expectLater(
          () => rustAutoOpaqueTwoArgsTwinSyncSse(a: obj, b: obj),
          throwsA(_duplicateMoveErrorMatcher()),
        );
        expect(obj.isDisposed, false);
        await futurizeVoidTwinSyncSse(
          rustAutoOpaqueArgOwnTwinSyncSse(arg: obj, expect: 10),
        );
      });
    });
    ...
    group('arg ref mut', () {
      test('can be called', () async {
        final obj = await rustAutoOpaqueReturnOwnTwinSyncSse(initial: 100);
        await futurizeVoidTwinSyncSse(
          rustAutoOpaqueArgMutBorrowTwinSyncSse(
  ...
  test('static method', () async {
    ...
    await futurizeVoidTwinSyncSse(
      NonCloneSimpleTwinSyncSse.staticMethodArgMutBorrowTwinSyncSse(arg: obj),
    );
    await futurizeVoidTwinSyncSse(
  ...
  group('Explicit rust-auto-opaque types', () {
    ...
    test('it can be inside a struct used as argument', () async {
      final obj = await rustAutoOpaqueExplicitReturnTwinSyncSse(initial: 100);
      await futurizeVoidTwinSyncSse(
        rustAutoOpaqueExplicitStructTwinSyncSse(
  ...
  group('borrow + borrow', () {
    test('when same object', () async {
      final obj = await rustAutoOpaqueReturnOwnTwinSyncSse(initial: 100);
      expect(
        await rustAutoOpaqueBorrowAndBorrowTwinSyncSse(a: obj, b: obj),
        200,
      );
    });
