...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('std any opaque call', () async {
    final opaque = await createAnyOpaqueTwinRustAsync();
    expect(await runAnyOpaqueTwinRustAsync(opaque: opaque), true);
  });
  ...
  test('call after dispose', () async {
    ...
    data.dispose();
    await expectLater(
      () => runOpaqueTwinRustAsync(opaque: data),
  ...
  test('create array of opaque type', () async {
    ...
    for (var v in data) {
      ...
      v.dispose();
      await expectLater(
        () => runOpaqueTwinRustAsync(opaque: v),
        throwsA(isA<DroppableDisposedException>()),
      );
    }
  });
  ...
  test('create enums of opaque type', () async {
    ...
    await expectLater(
      () => runEnumOpaqueTwinRustAsync(opaque: data[4]),
      throwsA(isA<DroppableDisposedException>()),
    );
  });
  ...
  test('opaque field', () async {
    ...
    data.first.dispose();
    await expectLater(
      () => runOpaqueTwinRustAsync(opaque: data.first),
      throwsA(isA<DroppableDisposedException>()),
    );
    await expectLater(
      () => runNestedOpaqueTwinRustAsync(opaque: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    expect(
      await runOpaqueTwinRustAsync(opaque: data.second),
  ...
  test('array', () async {
    ...
    await expectLater(
      () => opaqueArrayRunTwinRustAsync(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
  ...
  test('vec', () async {
    ...
    await expectLater(
      () => opaqueVecRunTwinRustAsync(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
