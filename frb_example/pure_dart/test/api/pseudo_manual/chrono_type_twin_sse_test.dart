...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('DateTime<Utc>', () async {
    ...
    final resp = await datetimeUtcTwinSse(d: date);
  ...
  test('DateTime<Local>', () async {
    ...
    final resp = await datetimeLocalTwinSse(d: date);
    expect(resp.year, date.year);
    expect(resp.month, date.month);
    expect(resp.day, date.day);
    expect(resp.hour, date.hour);
  ...
  test('NaiveDate', () async {
    final date = DateTime.utc(2022, 09, 10, 20, 48, 53, 123, 456);
    final resp = await naivedateTwinSse(d: date);
    expect(resp.year, date.year);
    expect(resp.month, date.month);
    expect(resp.day, date.day);
    expect(resp.hour, date.hour);
  ...
  test('NaiveDateTime', () async {
    final date = DateTime.utc(2022, 09, 10, 20, 48, 53, 123, 456);
    final resp = await naivedatetimeTwinSse(d: date);
    expect(resp.year, date.year);
    expect(resp.month, date.month);
    expect(resp.day, date.day);
    expect(resp.hour, date.hour);
  ...
  test('Empty DateTime', () async {
    final resp = await optionalEmptyDatetimeUtcTwinSse(d: null);
    expect(resp, null);
  });
  ...
  test('List<Duration>', () async {
    ...
    final durations = await handleTimestampsTwinSse(
  ...
  test('List<DateTime>', () async {
    ...
    final now = DateTime.now();
    final result =
        await handleDurationsTwinSse(durations: expected, since: now);
    expect(result, expected.map(now.subtract));
  });

  test('Combined Chrono types', () async {
    final test = await testChronoTwinSse();
  ...
  test('combined chrono types precise', () async {
    ...
    final result = await testPreciseChronoTwinSse();
