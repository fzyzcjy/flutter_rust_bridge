...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  test('Web MoiArc release completes after pool contention', () async {
    await Future<void>.sync(reproduceMoiArcReleaseContentionTwinSyncSse);
    final deadline = DateTime.now().add(const Duration(seconds: 5));
    while (await moiArcContentionValueDropCountTwinSyncSse() == 0 &&
        DateTime.now().isBefore(deadline)) {
      await Future<void>.delayed(const Duration(milliseconds: 10));
    }
    expect(await moiArcContentionValueDropCountTwinSyncSse(), 1);
  }, skip: !kIsWeb);
  ...
  test('call after dispose', () async {
    ...
    data.dispose();
    await expectLater(
      () => runOpaqueTwinSyncSse(opaque: data),
  ...
  test('create enums of opaque type', () async {
    ...
    await expectLater(
      () => runEnumOpaqueTwinSyncSse(opaque: data[4]),
      throwsA(isA<DroppableDisposedException>()),
    );
  });
  ...
  test('opaque field', () async {
    ...
    data.first.dispose();
    await expectLater(
      () => runOpaqueTwinSyncSse(opaque: data.first),
      throwsA(isA<DroppableDisposedException>()),
    );
    await expectLater(
      () => runNestedOpaqueTwinSyncSse(opaque: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    expect(
      await runOpaqueTwinSyncSse(opaque: data.second),
  ...
  test('array', () async {
    ...
    await expectLater(
      () => opaqueArrayRunTwinSyncSse(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
  ...
  test('vec', () async {
    ...
    await expectLater(
      () => opaqueVecRunTwinSyncSse(data: data),
      throwsA(isA<DroppableDisposedException>()),
    );
    data[1].dispose();
  });
