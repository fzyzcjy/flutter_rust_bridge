...
Future<void> main({bool skipRustLibInit = false}) async {
  ...
  addTestsIdentityFunctionCall(funcEnumWithItemTupleTwinSync, [
    ...
    EnumWithItemTupleTwinSync.b(200),
  ]);
  ...
  addTestsIdentityFunctionCall(funcEnumWithFreezedOverrideTwinSync, [
    const EnumWithFreezedOverrideTwinSync.empty(),
    const EnumWithFreezedOverrideTwinSync.named(count: 42),
    const EnumWithFreezedOverrideTwinSync.named(),
  ]);

  group('example-based tests', () {
    test('dart call handleReturnEnum', () async {
      expect(
        await handleReturnEnumTwinSync(input: "Tuesday"),
    ...
    test('dart call multiplyByTen()', () async {
      ...
      expect(
        (SpeedTwinSync_GPS).toString(),
        'SpeedTwinSync_GPS',
        skip: skipMinified,
      );
      expect(
        (DistanceTwinSync_Unknown).toString(),
        'DistanceTwinSync_Unknown',
        skip: skipMinified,
      );
      expect(
        (DistanceTwinSync_Map).toString(),
