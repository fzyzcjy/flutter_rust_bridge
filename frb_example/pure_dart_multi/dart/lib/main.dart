...
void main(List<String> args) async {
  ...
  test('dart call primitiveU32', () async {
    expect(await api1.primitiveU32(myU32: 0xff112233), 0xfe112233);
  });

  test('dart call handleReturnUnit', () async {
    await api1.handleReturnUnit();
  });

  test('dart call handleString', () async {
    expect(await api1.handleString(s: "Hello, world!"),
        "Hello, world!Hello, world!");
  });

  test('dart call handleVecU8', () async {
    final len = 100000;
    expect(await api1.handleVecU8(v: Uint8List.fromList(List.filled(len, 127))),
        Uint8List.fromList(List.filled(len * 2, 127)));
  });

  test('dart call handleVecOfPrimitive', () async {
    final n = 10000;
    final resp = await api1.handleVecOfPrimitive(n: n);
  ...
  test('dart call handleZeroCopyVecOfPrimitive', () async {
    final n = 10000;
    final resp = await api1.handleZeroCopyVecOfPrimitive(n: n);
    expect(resp.uint8List, Uint8List.fromList(List.filled(n, 42)));
    expect(resp.int8List, Int8List.fromList(List.filled(n, 42)));
    expect(resp.uint16List, Uint16List.fromList(List.filled(n, 42)));
    expect(resp.int16List, Int16List.fromList(List.filled(n, 42)));
  ...
  test('dart call handleStruct', () async {
    final structResp = await api1.handleStruct(
  ...
  test('dart call handleNewtype', () async {
    final newtypeResp = await api1.handleNewtype(arg: NewTypeInt(field0: 42));
    expect(newtypeResp.field0, 84);
  });

  test('dart call handleListOfStruct', () async {
    final listOfStructResp = await api1.handleListOfStruct(
  ...
  test('dart call handleStringList', () async {
    final names = await api1.handleStringList(names: ['Steve', 'Bob', 'Alex']);
  ...
  test('dart call handleComplexStruct', () async {
    ...
    final complexStructResp =
        await api1.handleComplexStruct(s: _createMyTreeNode(arrLen: arrLen));
  ...
  test('dart call handle_sync_return', () async {
    expect(api1.handleSyncReturn(mode: 'NORMAL'), List.filled(100, 42));

    for (final mode in ['RESULT_ERR', 'PANIC']) {
      try {
        api1.handleSyncReturn(mode: mode);
  ...
  test('dart call handle_stream', () async {
    final stream = api1.handleStream(arg: 'hello');
    var cnt = 0;
  ...
  test('dart call handleOptionalReturn', () async {
    expect((await api1.handleOptionalReturn(left: 1, right: 1))!, 1);
    expect(await api1.handleOptionalReturn(left: 2, right: 0), null);
  });

  test('dart call handleOptionalStruct', () async {
    {
      expect(await api1.handleOptionalStruct(), null);
    }

    {
      final message = 'Hello there.';
      final ret = await api1.handleOptionalStruct(document: message);
      if (ret == null)
  ...
  test('dart call handleOptionalIncrement', () async {
    expect(await api1.handleOptionalIncrement(), null);
    {
      var ret = await api1.handleOptionalIncrement(
      ...
      for (var i = 1; i < loopFor; i++) {
        ret = await api1.handleOptionalIncrement(opt: ret);
      }
  ...
  test('dart call handleIncrementBoxedOptional', () async {
    {
      expect(await api1.handleIncrementBoxedOptional(), 42);
    }

    {
      ...
      for (var i = 0; i < loopFor; i++) {
        ret = await api1.handleIncrementBoxedOptional(opt: ret);
      }
      expect(ret, loopFor);
    }
  });
  ...
  test('dart call handleOptionBoxArguments', () async {
    print(await api1.handleOptionBoxArguments());

    {
      final optional10 = await api1.handleOptionBoxArguments(
        boolbox: true,
        structbox: await api1.handleOptionalIncrement(
            opt: ExoticOptionals(attributesNullable: [])),
      );
      print(optional10);
    }
  ...
  test('dart call handleReturnEnum', () async {
    expect(await api1.handleReturnEnum(input: "Tuesday"), Weekdays.Tuesday);
    expect(await api1.handleReturnEnum(input: "Foreverday"), null);
  });

  test('dart call handleEnumParameter', () async {
    expect(await api1.handleEnumParameter(weekday: Weekdays.Saturday),
        Weekdays.Saturday);
  });

  test('dart call handleEnumStruct', () async {
    expect(await api1.handleEnumStruct(val: Empty()), Empty());
    expect(
      await api1.handleEnumStruct(
        val: Primitives(int32: 0, float64: 1, boolean: false),
      ),
      Primitives(int32: 1, float64: 2, boolean: true),
    );
    expect(
      await api1.handleEnumStruct(val: Optional(null, 0)),
      Optional(null, 1),
    );
    expect(
      await api1.handleEnumStruct(val: Buffer(Uint8List.fromList([]))),
      Buffer(Uint8List.fromList([1])),
    );
    expect(
      await api1.handleEnumStruct(val: Enums(Weekdays.Monday)),
      Enums(Weekdays.Tuesday),
    );
    expect(
      await api1.handleEnumStruct(val: Nested(Empty(), 0)),
      Nested(Empty(), 1),
    );
  });
  ...
  test('dart call useImportedStruct()', () async {
    expect(
      await api1.useImportedStruct(myStruct: MyStruct(content: false)),
      false,
    );
    expect(
      await api1.useImportedStruct(myStruct: MyStruct(content: true)),
      true,
    );
  });
  ...
  test('dart call useImportedEnum()', () async {
    expect(
      await api1.useImportedEnum(myEnum: MyEnum.False),
      false,
    );
    expect(
      await api1.useImportedEnum(myEnum: MyEnum.True),
      true,
    );
  });

  test('dart call getAppSettings()', () async {
    var settings = await api1.getAppSettings();
  ...
  test('dart call getMessage()', () async {
    var message = await api1.getMessage();
  ...
  test('loop and call many times', () async {
    ...
    for (var i = 0; i < 500; ++i) {
      obj = await api1.handleComplexStruct(s: obj);
    }
  });

  test('dart call getArray()', () async {
    expect(await api1.getArray(), [1, 2, 3, 4, 5]);
  });

  test('dart call getComplexArray()', () async {
    final points = await api1.getComplexArray();
  ...
  test('dart call getUsize', () async {
    expect(await api1.getUsize(u: 2), 2);
  });
  ...
  test('dart call next_user_id to test metadata annotations', () async {
    ...
    expect(await api1.nextUserId(userId: userId), UserId(value: 12));
  });

  test('dart register event listener & create event after delayed future',
      () async {
    bool listenerCalled = false;
    api1.registerEventListener().listen((e) {
      listenerCalled = true;
    });

    await Future.delayed(const Duration(milliseconds: 10));
    await api1.createEvent();
