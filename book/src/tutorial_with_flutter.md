...
* Install `ffigen`: either `dart pub add --dev ffigen`, `dart pub add ffi` (dart project) or `flutter pub add --dev ffigen`, `flutter pub add ffi` (flutter project) and [install LLVM](https://pub.dev/packages/ffigen#installing-llvm) which is required by `ffigen`.
