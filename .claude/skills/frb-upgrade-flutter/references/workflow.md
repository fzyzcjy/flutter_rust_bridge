...
Use official Flutter sources. Record:

- Target Flutter version
- Bundled Dart version
- Release date
- Release-note items likely to affect FRB
...
After the Dockerfile change reaches the publish branch, publish:

```shell
gh workflow run publish_dev_docker.yaml --ref master
```

Verify both `linux/amd64` and `linux/arm64`:
