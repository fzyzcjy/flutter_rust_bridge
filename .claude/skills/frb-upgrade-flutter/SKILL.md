...
- Use `frb-dev-env` and `frb-docker` to build a fresh image and recreate the per-worktree container. Do not validate
  the upgrade in a stale container or use ad hoc bind mounts that bypass the worktree-aware environment.
...
- Use the fresh local Docker environment from `frb-dev-env`.
