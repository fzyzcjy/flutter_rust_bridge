...
impl<'a> Parser<'a> {
    ...
    fn try_parse_arc(&self, ty: &str) -> Option<ApiType> {
        lazy_static! {
            static ref CAPTURE_ARC: GenericCapture = GenericCapture::new("Arc");
        }

        CAPTURE_ARC.captures(ty).map(|ty| ApiType::Arc(ApiArc(ty)))
    }
