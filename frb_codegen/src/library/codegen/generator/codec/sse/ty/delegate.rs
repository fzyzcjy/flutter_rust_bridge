        ...
        let inner_expr = match lang {
            Lang::DartLang(_) => match &self.mir {
                ...
                MirTypeDelegate::CustomSerializer(mir) => {
                    mir.codec_info.dart_encode.replace("{}", "self")
                }
            },
            Lang::RustLang(_) => match &self.mir {
                ...
                MirTypeDelegate::CustomSerializer(mir) => {
                    format!("{}(self)", mir.codec_info.rust_encode_function.rust_style())
                }
            },
        };
        ...
        let wrapper_expr = match lang {
            Lang::DartLang(_) => {
                match &self.mir {
                    ...
                    MirTypeDelegate::CustomSerializer(mir) => {
                        mir.codec_info.dart_decode.replace("{}", "self")
                    }
                }
            }
            Lang::RustLang(_) => match &self.mir {
                ...
                | MirTypeDelegate::CastedPrimitive(_) => return None,
                MirTypeDelegate::CustomSerializer(mir) => {
                    format!("{}(self)", mir.codec_info.rust_decode_function.rust_style())
                }
            },
        };
