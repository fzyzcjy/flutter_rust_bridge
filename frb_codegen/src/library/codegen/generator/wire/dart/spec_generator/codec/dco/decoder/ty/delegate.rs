...
impl<'a> WireDartCodecDcoGeneratorDecoderTrait for DelegateWireDartCodecDcoGenerator<'a> {
    fn generate_impl_decode_body(&self) -> String {
        match &self.ir {
            ...
            // }
            IrTypeDelegate::Uuid => {
                "return UuidValue.fromByteList(dco_decode_list_prim_u_8(raw));".to_owned()
            }
            ...
            IrTypeDelegate::Map(_) => "return Map.fromEntries((raw as List<dynamic>).map((e) => MapEntry(e[0], e[1])));".to_owned(),
            IrTypeDelegate::Set(_) => "return Set.from(raw as List<dynamic>);".to_owned(),
        }
    }
}
