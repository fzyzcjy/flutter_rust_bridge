...
impl<'a> WireDartCodecDcoGeneratorDecoderTrait for FutureWireDartCodecDcoGenerator<'a> {
    // the function signature is not covered while the whole body is covered - looks like a bug in coverage tool
    // frb-coverage:ignore-start
    fn generate_impl_decode_body(&self) -> String {
        // frb-coverage:ignore-end
        match &*self.mir.output {
            StructRef(_)
            | DartOpaque(_)
            | RustOpaque(_)
            ...
            | Delegate(MirTypeDelegate::Array(_) | MirTypeDelegate::PrimitiveEnum { .. }) => {
                format!("return dco_decode_{}(raw);", self.mir.output.safe_ident())
            }
