...
use crate::codegen::generator::wire::rust::spec_generator::wire2api::ty::general_list::{
...
impl<'a> WireRustGeneratorWire2apiTrait for DelegateWireRustGenerator<'a> {
    ...
    fn generate_impl_wire2api_jsvalue_body(&self) -> Option<std::borrow::Cow<str>> {
        Some(match &self.ir {
            ...
            IrTypeDelegate::TimeList(_) =>
                "self.unchecked_into::<wasm_bindgen::js_sys::BigInt64Array>().to_vec().into_iter().map(Wire2Api::wire2api).collect()".into(),
            IrTypeDelegate::Uuid | IrTypeDelegate::Uuids => {
                "self.unchecked_into::<wasm_bindgen::js_sys::Uint8Array>().to_vec().into_boxed_slice().wire2api()"
                    .into()
            }
