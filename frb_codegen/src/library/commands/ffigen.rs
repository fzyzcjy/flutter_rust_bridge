...
mod tests {
    ...
    fn parse_test_config(
        ...
        function_rename: Option<&HashMap<String, String>>,
    ) -> FfigenCommandConfig {
        parse_config(&FfigenToFileArgs {
            c_path: Path::new("/tmp/header.h"),
            dart_path: Path::new("/tmp/bindings.dart"),
            dart_class_name: "Bindings",
            llvm_path: &[PathBuf::from("/llvm/one"), PathBuf::from("/llvm/two")],
            llvm_compiler_opts,
            dart_root: Path::new("/tmp/dart"),
            function_rename,
        })
    }

    /// Returns no warning for a successful, clean ffigen invocation.
    #[test]
    fn test_handle_output_when_normal() {
        let result = handle_output(true, "", "");
        assert_eq!(result.unwrap(), None);
    }

    /// Preserves unexpected severe output as a warning.
    #[test]
    fn test_handle_output_when_has_severe_should_warn() {
    ...
    /// Ignores ffigen's known benign severe diagnostics.
    #[test]
    fn test_handle_output_when_has_severe_but_known_no_problem() {
        let result = handle_output(true, "One line
...
Another line", "");
    ...
    /// Warns about headers that ffigen cannot preprocess.
    #[test]
    fn test_handle_output_when_has_severe_stdbool_not_found() {
...
Another line"#,
    ...
    /// Explains the LLVM lookup failure distinctly from generic ffigen errors.
    #[test]
    fn test_handle_output_when_cannot_find_llvm_should_fail() {
        let result = handle_output(
            false,
    ...
    /// Builds the complete default ffigen configuration without launching Dart.
    #[test]
    fn test_parse_config_defaults_and_json_shape() {
        let config = parse_test_config("", None);

        assert_eq!(config.output, PathBuf::from("/tmp/bindings.dart"));
        assert_eq!(config.name, "Bindings");
        assert_eq!(config.description, "generated by flutter_rust_bridge");
        assert_eq!(
            config.headers.entry_points,
            vec![PathBuf::from("/tmp/header.h")]
        );
        assert_eq!(
            config.headers.include_directives,
            vec![PathBuf::from("/tmp/header.h")]
        );
        assert_eq!(config.comments, Some(false));
        assert!(config.compiler_opts.is_empty());
        assert_eq!(config.functions.rename, None);
        assert!(config.ignore_source_errors);

        let value = serde_json::to_value(config).unwrap();
        assert_eq!(value["output"], "/tmp/bindings.dart");
        assert_eq!(value["headers"]["entry-points"][0], "/tmp/header.h");
        assert_eq!(value["llvm-path"][1], "/llvm/two");
        assert_eq!(value["comments"], false);
        assert_eq!(
            value["preamble"],
            "// ignore_for_file: camel_case_types, non_constant_identifier_names, avoid_positional_boolean_parameters, annotate_overrides, constant_identifier_names"
        );
        assert_eq!(value["ignore-source-errors"], true);
        assert!(value.get("ignore_source_errors").is_none());
    }

    /// Keeps compiler overrides and function renames in the generated command configuration.
    #[test]
    fn test_parse_config_applies_overrides() {
        let renames = HashMap::from([("old_name".to_owned(), "newName".to_owned())]);
        let config = parse_test_config("-DTEST=1", Some(&renames));

        assert_eq!(config.compiler_opts, vec!["-DTEST=1"]);
        assert_eq!(config.functions.rename, Some(renames));
        let value = serde_json::to_value(config).unwrap();
        assert_eq!(value["compiler-opts"][0], "-DTEST=1");
        assert_eq!(value["functions"]["rename"]["old_name"], "newName");
    }
}
