...
#[cfg(test)]
mod tests {
    use super::generate_frb_rust_extra_code;

    /// Generated cbindgen input includes every FFI dispatcher mode.
    #[test]
    fn rust_extra_code_includes_all_dispatchers() {
        let code = generate_frb_rust_extra_code();

        assert!(code.contains("frb_generated_io_extern_func"));
        assert!(code.contains("pde_ffi_dispatcher_primary_impl"));
        assert!(code.contains("pde_ffi_dispatcher_sync_impl"));
    }
}
